#!/usr/bin/env python
'''
compare k-mer frequency(change complement kmer apperance to 0.5)
'''
...
def ratio(freq):
    ...
    proportion={key: (value / total_value) for key, value in freq.items()}
    return proportion
...
def fc(dic_ar, dic_br):
    ...
    # Set default value to 0.5 for keys in dic_a that are not in dic_b
    for k in set(dic_ar.keys()) - set(dic_br.keys()):
        result[k] = '{:.3f}'.format(dic_a[k] / (0.5/sum(dic_br.values())))

    # Set default value to 0.5 for keys in dic_b that are not in dic_a
    for k in set(dic_b.keys()) - set(dic_a.keys()):
        result[k] = '{:.3f}'.format( (0.5/sum(dic_ar.values())) / dic_b[k])
