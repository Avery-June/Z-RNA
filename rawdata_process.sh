...
--genomeFastaFiles ${refpath}/plasmid.fa     \
--sjdbGTFfile ${refpath}/plasmid.gtf \
--sjdbOverhang 100
...
ls *Aligned.sortedByCoord.out.bam|while read id;do samtools idxstats ${id} | cut -f1 | grep -v "^chrPlasmid$" >> all_chromosomes.txt;done
